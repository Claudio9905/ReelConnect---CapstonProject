import { updateMyAvatarProfile } from "../redux/actions/actions";
import { Button, Form, FormControl, FormGroup, Modal } from "react-bootstrap";
import { useState } from "react";
import type { AppDispatch } from "../redux/store";
import { useDispatch } from "react-redux";

type ModaleAvatarProfile = {
  onClose: () => void;
  onShow: boolean;
};

const ModaleEditAvatarProfile: React.FC<ModaleAvatarProfile> = ({
  onClose,
  onShow,
}) => {
  ...
  const editSubmit = (e: React.FormEvent) => {
    ...
    dispatch(updateMyAvatarProfile(formData));
  };
  return (
    <>
      <Modal id="modal-main-profile" show={onShow} onHide={onClose}>
        <Modal.Header id="modal-header-profile">
          <Modal.Title>Modifica il tuo avatar del profilo</Modal.Title>
        </Modal.Header>
...
export default ModaleEditAvatarProfile;
